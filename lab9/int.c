...
int kcinth()
{
	...
	switch(a){
		...
		case 17 : r = usleep(b);	break;
		case 18 : r = ksin(b);	break;
		case 19 : r = ksout(b);	break;
	}
...
int kps()	//Print out all the processes and the inforamtion about them
{
	...
	int i = 0;
	printf("# \tName\tStatus\tPID\tPPID\tTime\n");
	for (i = 0; i < NPROC; i++){
		printf("P%d\t%s\t%s\t%d\t%d\t%d\n", i, proc[i].name, statuses[proc[i].status], proc[i].pid, proc[i].ppid, proc[i].time);
	}
}
...
int usleep(int sleepEvent){ 
	running->time = sleepEvent;	//How long do we want to sleep?
	running->event = running;	//What event value?
	running->status = SLEEP;	//Set the status
	enqueue(&sleepList, running);	//Put it in the sleep list
	tswitch();	//Go to the next proc
}
...
//USED FOR SERIAL TO TERMINAL (QEMU)
int ksin(char *y)
{
  ...
  //We could also use 1, but in this case we will just use 0
  //
  //Get string from SERIAL port
  length = sgetline(0, inbuf);

  //Wrtie string to umode
  while(inbuf[i] != '\0'){
	put_byte(inbuf[i++], running->uss, y++);
  }

  return length;
}

//USED FOR TERMINAL TO SERIAL
int ksout(char *y)
{
  ...
  //Read string from UMODE to the outbuf, call sputline
  while(i < 64){
  ...
  //Wrtie string to serial port (aka tty)
  sputline(tty, outbufs);
}
