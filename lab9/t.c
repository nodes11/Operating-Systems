...
PROC *freeList, *readyQueue, *sleepList, *timerList;
...
int scheduler()
{
	...
	//Set the time back to 5 after it sleeps!
	running->time = 5;
...
main()
{
	vid_init();	//Intialize the screen with blanks
	...
	//Set the serial port handler in the interrupt vector and intialize it
    set_vector(12, s0inth);
    sinit();
  

    lock();
	set_vector(8,tinth);	//Place the timer interupt handler in the 8th vector spot
    timer_init();
