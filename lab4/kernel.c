...
********************************************************************************/
	...
	if (filename){
		...
		for (i = 3; i < 11; i++){
			put_word(0,p->uss,0x1000-i*-2);	//Fill the low end (int80) registers with 0's. Make sure offset is correct (2 bytes!)!
		}
		
		//SET ALL THE STACK SEGMENTS!!!
		put_word(p->uss, p->uss, 0x1000-24);	//uDS | starting from the top of the stack on going back (uDS is at the low end)
		put_word(p->uss, p->uss, 0x1000-22);	//uES same ^
		put_word(p->uss, p->uss, 0x1000-4);		//uCS same ^^
		put_word(0x200,p->uss,0x1000-2);		//flag
